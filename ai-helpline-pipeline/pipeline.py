...
class HelplinePipeline:
	...
	def process_audio(
		...
		if effective_source != "en":
			self.logger.info("Step 2: Translating query to en...")
			tr = self.sarvam.translate(stt.text, source_lang=effective_source, target_lang="en")
		...
		if effective_source != "en":
			...
			back = self.sarvam.translate(llm_response_en, source_lang="en", target_lang=effective_source)
